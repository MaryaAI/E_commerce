            ...
            <tbody>
                @foreach($users as $user)
                    <tr>
                        ...
                        <td>
                            <form class="ml-4 form-inline" method="POST" action="{{ route('users.show', $user) }}" style="display:inline-block">
                                @method('patch')
                                @csrf
                            ...
                            <form method="POST" action="{{ route('users.show', $user) }}" style="display:inline-block">
                                @method('delete')
                                @csrf
                                @if(auth()->user() != $user)
